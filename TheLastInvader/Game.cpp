...
public olc::PixelGameEngine
{
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
        if (lives > 0) {
            ...
            int player_x_min = player_movement.position.x;
            int player_y_min = player_movement.position.y;
            ...
            for (auto& missile: enemy_bullets) {
                if (missile.position.x >= player_x_min && missile.position.x <= player_x_max && missile.position.y >= player_y_min && missile.position.y <= player_y_max){
                    ...
                    // TODO(gareth): What do we do on a hit? Play a sound and lose a life?
                    // Should the player have some form of durability?
            ...
            for (auto& bullet : enemy_bullets) {
                ...
                DrawLine(bullet.position.x, bullet.position.y, middle.x, middle.y, olc::RED);DrawLine(bullet.position.x, bullet.position.y, back.x, back.y, olc::DARK_RED);
                Draw(bullet.position.x, bullet.position.y, olc::YELLOW);
